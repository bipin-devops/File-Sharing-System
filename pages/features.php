   ...
   <div class="courses_box1">
	   <div class="container">
		...
		<div class="col-md-8 detail">
	   	 	<?php
	   	 		$sqlcount=" select count(*) as totalCount from tbl_file ";
...
$limit = 'LIMIT ' .($pn - 1) * $itemsPerPage .',' .$itemsPerPage;
                            ...
                            $news="select * from tbl_file where status='active' ORDER BY upload_date ASC $limit";
                            $result=mysqli_query($connection, $news);
                            if(mysqli_num_rows($result)>0) {
                            while ($row=mysqli_fetch_assoc($result)) {
	       ...
	       <div class="event-page">
	       	 <div class="row">
	       	 	<div class="col-xs-4 col-sm-4">
	       	 	  <div class="event-img">
	       	 		<?php echo"<a href='single.php?id=$row[id]'><img src='../images/$row[images]' height=200 width=250/></a>";?>
	       	 		<div class="over-image"></div>
	       	 	  </div>
	       	 	</div>
	       	 	<div class="col-xs-8 col-sm-8 event-desc">
	       	 		<h2><?php echo"<a href='single.php?id=$row[id]'>$row[file_name]</a>";?></h2>
	       	 	    <div class="event-info-text">
	       	 		   <div class="event-info-middle"><p style="display:inline;"><span class="event-bold">Posted By : &nbsp;</span></p><a href="#"><?=$r['username']?></a>
						<p><span class="event-bold">Date : &nbsp;</span><?=$row['upload_date']?></p>
						<p><span class="event-bold"> 
								  <?php 
				                  $string=$row['description'];
				                  if (strlen($string) > 179) {
				                        $trimstring = substr($string, 0, 179);
				                        } else {
				                        $trimstring = $string;
				                               }
				                  echo $trimstring;
                           ...
                           ?>
                              <?php echo"<a href='single.php?id=$row[id]'>  Download</a>";?>
                          <?php
                            } else {
                          ?>
