...
$limit = 'LIMIT ' .($pn - 1) * $itemsPerPage .',' .$itemsPerPage;
                            ...
                            $news="SELECT * FROM `tbl_file` where status='active' order by rand()   $limit";
                            $result=mysqli_query($connection, $news);
                            if(mysqli_num_rows($result)>0) {
                            while ($row=mysqli_fetch_assoc($result)) {
                ?>
            ...
            <div class="col-xs-8 col-sm-8 event-desc">
              <h2><?php echo"<a href='single.php?id=$row[id]'>$row[file_name]</a>";?></h2>
                <div class="event-info-text">
                 <div class="event-info-middle"><p style="display:inline;"><span class="event-bold">Posted By : &nbsp;</span></p><a href="single.php">Username</a>
